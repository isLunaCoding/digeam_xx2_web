...
class indexController extends Controller
{
    ...
    {
        //最新
        $NA = Page::where('type', 'announcement')->where('open', 'Y')->where('created_at', '<=', date('Y-m-d H:i:s'))->orderby('top', 'desc')->orderby('new', 'desc')->orderBy('created_at', 'desc')->limit(5)->get();
        //活動
        $NB = Page::where('cate_id', 2)->where('open', 'Y')->where('created_at', '<=', date('Y-m-d H:i:s'))->orderby('top', 'desc')->orderBy('new', 'desc')->orderBy('created_at', 'desc')->limit(5)->get();
        //系統
        $NC = Page::where('cate_id', 3)->where('open', 'Y')->where('created_at', '<=', date('Y-m-d H:i:s'))->orderby('top', 'desc')->orderBy('new', 'desc')->orderBy('created_at', 'desc')->limit(5)->get();
