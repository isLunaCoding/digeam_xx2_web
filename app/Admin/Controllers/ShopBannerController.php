...
class ShopBannerController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->model()->where('type', 'shop')->orderBy('status', 'desc')->orderBy('sort', 'asc');
    ...
    protected function form()
    {
        ...
        $form->image('file_name', __('圖片'))->uniqueName()->move('upload/shop');
        $form->url('url', __('網址'));
        $form->select('status', __('狀態'))->options(['N' => '關閉', 'Y' => '開啟'])->default('N');
        $form->number('sort', __('排序'));
        $form->text('type', __('類型'))->default('shop')->readonly();
        return $form;
    }
}
