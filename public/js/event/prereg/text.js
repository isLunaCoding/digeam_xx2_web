...
function p2informationIn() {
    var p2informationarray = [
        "1.活動時間：即日起 ~ 《仙俠世界貳》上市後一週。",
        "2.註冊並登入DiGeam掘夢網平台帳號。",
        "3.填寫手機號碼，並同意接收遊戲相關簡訊。",
        "4.點擊「初出江湖」按鈕，即可完成活動。",
    ];
...
function p2noticeIn() {
    var p2noticearray = [
        "1.每組掘夢網會員帳號僅限參加一次事前預約活動。本活動僅限台港澳地區玩家參與。",
        "2.請確認您填寫的手機號碼是否正確，每個手機號碼僅可申請一次事前預約活動。",
        "3.本活動所提供之虛寶獎勵，皆為不可交易之性質，實際道具限制依遊戲內為準。領出前請務必留意角色ID，一經領取恕不提供轉移道具之服務。",
        "4.若因觸犯遊戲規章遭受凍結處分、個人線路不穩、個人操作不慎等，導致斷線、連線失敗等問題影響活動參與，活動將照常舉行，不另做補償。",
        "5.本公司有權檢視各參加者之活動參與行為及得獎情形是否涉嫌：人為操作、蓄意偽造、多開(重)帳號、短時間異常多筆參與行為、透過任何電腦程式參與活動、詐欺、任何違反會員系統服務合約及停權管理規章之情事者，或以任何其他不正常的方式意圖進行不實或虛偽活動參與行為，參加者因上述情形所獲得之活動資格及獎項，本公司得一概取消之。",
        "6.本活動各項辦法及規定，以活動網站公告及本公司官方最新說明為準。掘夢網股份有限公司擁有活動最終保留、變更、修正或撤回、取消獎項發送之權利，若因不可抗力之因素，本活動將有權隨時補充或修正，並以最新公告為主。",
    ];
...
function p2_success() {
    ...
    $(".popSText")
        .html(
            "<span>由衷地感謝您參與初出江湖活動，前往下一步完成更多任務，將有機會解鎖更多的獎勵。</span>"
        )
        .css({
            fontSize: "1.3rem",
        });
...
function p2_pre_safe_error() {
    ...
    $(".popSText")
        .html(
            '請勾選 我已閱讀且同意<a href="https://www.digeam.com/terms2"  target="_blank">隱私權政策</a>與活動相關注意事項。​​'
        )
        .css({
            fontSize: "1.8rem",
        });
...
function p3informationIn() {
    var p3informationarray = [
        "1.活動時間：即日起 ~ 《仙俠世界貳》上市後一週。",
        "2.活動期間內完成指定任務可獲得對應拜訪次數，部分任務於活動期間內僅可完成一次，可重複完成的任務將於每日00:00重置。",
        "3.完成任務取得拜訪次數後，點選【拜訪名士】即可進行結交名士。",
        "4.拜訪名士後，可確認該名士的能力值等相關介紹。",
        "5.累計拜訪次數達30次，可以從「仙道盟主沈仲陽」、「七花獸百花仙靈」、「愛之紅娘」三位名士中任選一位名士取代原先拜訪的名士，作為當前拜訪名士結果。",
        "6.可與拜訪的名士進行結義，進行結義後無法再更改結義名士，並可於後續遊戲內獲得選定結義名士的仙俠錄。",
    ];
...
function p3noticeIn() {
    var p3noticearray = [
        "1.此活動需完成事前預約活動才可參加。",
        "2.此活動的抽獎為機會中獎獎勵，玩家參與活動不代表即可獲得指定獎勵。",
        "3.玩家在拜訪名士後，再次進行拜訪名士將放棄前次抽取到的名士。",
        "4.結交名士活動最多只可獲得一次獎勵，且進行結義後即視為最終選定結果，無法再進行拜訪名士。",
        "5.本活動需要於主畫面點選結義按鈕選定最終獎勵，若因沒有進行結義導致無法取得獎勵，恕不進行補償。",
        "6.本活動所提供之虛寶獎勵，皆為不可交易之性質，實際道具限制依遊戲內為準。領出前請務必留意角色ID，一經領取恕不提供轉移道具之服務。",
        "7.若因觸犯遊戲規章遭受凍結處分、個人線路不穩、個人操作不慎等，導致斷線、連線失敗等問題影響活動參與，活動將照常舉行，不另做補償。",
...
function p4informationIn() {
    var p4informationarray = [
        "1.活動時間：即日起 ~ 《仙俠世界貳》上市後一週。",
        "2.活動期間內，<span>將於每日00:00開啟一輪新的賽事，每日登入活動頁面都可以遊玩一次熊貓賽跑。</span>",
        "3.進行熊貓賽跑時，將從熊貓酒仙、熊貓少林、熊貓船長三隻熊貓中選出一隻心儀的熊貓，若預測成功則可累計一次猜對次數，累計猜對達指定次數，可獲得對應獎勵。",
        "4.預測失敗也不用灰心，每日參與熊貓賽跑競猜活動，不論是否預測成功皆可累計競猜次數，累計參與活動達指定次數，可獲得對應獎勵。",
    ];
...
function p4noticeIn() {
    var p4noticearray = [
        "1.此活動需完成事前預約活動才可參加。",
        "2.熊貓賽跑活動將於每日00:00開啟一輪新的賽事，每日登入活動頁面僅可遊玩一次熊貓賽跑 。",
        "3.本活動所提供之虛寶獎勵，皆為不可交易之性質，實際道具限制依遊戲內為準。領出前請務必留意角色ID，一經領取恕不提供轉移道具之服務。",
        "4.若因觸犯遊戲規章遭受凍結處分、個人線路不穩、個人操作不慎等，導致斷線、連線失敗等問題影響活動參與，活動將照常舉行，不另做補償。",
        "5.本公司有權檢視各參加者之活動參與行為及得獎情形是否涉嫌：人為操作、蓄意偽造、多開(重)帳號、短時間異常多筆參與行為、透過任何電腦程式參與活動、詐欺、任何違反會員系統服務合約及停權管理規章之情事者，或以任何其他不正常的方式意圖進行不實或虛偽活動參與行為，參加者因上述情形所獲得之活動資格及獎項，本公司得一概取消之。",
        "6.本活動各項辦法及規定，以活動網站公告及本公司官方最新說明為準。掘夢網股份有限公司擁有活動最終保留、變更、修正或撤回、取消獎項發送之權利，若因不可抗力之因素，本活動將有權隨時補充或修正，並以最新公告為主。",
...
function p4awardIn() {
    var p4award1array = {
        ...
        info: [
            "獎勵說明",
            "玄獸技能升級消耗的道具",
            "用以強化玄獸/仙尊/仙童根骨等級，<br>最高提升至10",
            "獲得琉璃竹後，<br>可至遊戲中跟NPC馴靈小仙兌換",
            "用以強化玄獸/仙尊/仙童根骨等級，<br>從10開始最高可提升至16",
            "獲得琉璃竹後，<br>可至遊戲中跟NPC馴靈小仙兌換",
        ],
    };
    var p4award2array = {
        ...
        info: [
            "獎勵說明",
            "鑲嵌效果：體質+63<br>鑲嵌部位:衣服、鞋子、腰帶、護腿、護腕、翅膀<br>一件裝備只能鑲嵌一顆同類寶石",
            "鑲嵌效果：體質+63 精神+63<br>鑲嵌部位:衣服、鞋子、腰帶、護腿、護腕、翅膀<br>一件裝備只能鑲嵌一顆同類寶石",
            "使用後可增加玄獸經驗值",
            "用以強化玄獸/仙尊/仙童根骨等級，<br>最高提升至10",
    ...
    for (j = 0; j < 6; j++) {
        if (j == 0) {
            p4award1str +=
                '<tr><td style="width: 15%;background-color:#d9ebff;">' +
...
var p6textarray = {
    ...
    text2: [
        "遠程法系輸出，可以控制大量敵人，並給予傷害，但是血防較為薄弱，容易被集火擊殺。",
...
//p7"活動規則"注意事項
var p7rulesarray = [
    "1.活動時間：即日起至《仙俠世界貳》上市後一週15:00截止。",
    "2.本活動所提供之虛寶獎勵，皆為不可交易之性質，實際道具限制依遊戲內為準。領出前請務必留意角色ID，一經領取恕不提供轉移道具之服務。",
    "3.各項活動獎勵需於遊戲正式開服後，於官網內的「領獎專區」進行領取，獎勵才會發送至對應角色信箱內。",
    "4.各項活動參加、得獎資格與獎項不得轉讓或贈與第三人。",
    "5.若因觸犯遊戲規章遭受凍結處分、個人線路不穩、個人操作不慎等，導致斷線、連線失敗等問題影響活動參與，活動將照常舉行，不另做補償。",
    "6.本公司有權檢視各參加者之活動參與行為及得獎情形是否涉嫌：人為操作、蓄意偽造、多開(重)帳號、短時間異常多筆參與行為、透過任何電腦程式參與活動、詐欺、任何違反會員系統服務合約及停權管理規章之情事者，或以任何其他不正常的方式意圖進行不實或虛偽活動參與行為，參加者因上述情形所獲得之活動資格及獎項，本公司得一概取消之。",
    "7.本活動各項辦法及規定，以活動網站公告及本公司官方最新說明為準。掘夢網股份有限公司擁有活動最終保留、變更、修正或撤回、取消獎項發送之權利，若因不可抗力之因素，本活動將有權隨時補充或修正，並以最新公告為主。",
];
